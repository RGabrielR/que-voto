...
export const housingResults = {
  async chooseRandomUrlAndFetch() {
    try {
  ...
  async chooseRandomElement(array) {
    const randomIndex = Math.floor(Math.random() * array.length);
    return array[randomIndex];
  },
  async populateOneResult(_parent, _params, _context) {
    try {
      let elements = await this.chooseRandomUrlAndFetch();
      const apartmentData = {};
  ...
  async chooseToPlay(_parent, _params, _context) {
    try {
      const Ids = await prisma.guess_apartment.findMany({
        select: {
          id: true,
        },
      });
      console.log("Ids", Ids, "Ids.length", Ids.length, "Ids[0]", Ids[0]);
      const randomIndex = Math.floor(Math.random() * Ids.length);
      console.log("randomIndex", randomIndex);

      return await prisma.guess_apartment.findUnique({
        where: {
